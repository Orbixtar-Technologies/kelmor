...
echo "== portals =="
curl -sk -o /dev/null -w 'server:%{http_code}\n' https://127.0.0.1:2087/ | grep -q 200 || \
  curl -sS -o /dev/null -w 'server:%{http_code}\n' http://127.0.0.1:2087/ | grep -q 200
curl -sk -o /dev/null -w 'account:%{http_code}\n' https://127.0.0.1:2083/ | grep -q 200 || \
  curl -sS -o /dev/null -w 'account:%{http_code}\n' http://127.0.0.1:2083/ | grep -q 200
curl -sk https://127.0.0.1:2087/ | grep -q 'Kelmor Director' || curl -sS http://127.0.0.1:2087/ | grep -q 'Kelmor Director'
curl -sk https://127.0.0.1:2083/ | grep -q 'Kelmor Control' || curl -sS http://127.0.0.1:2083/ | grep -q 'Kelmor Control'
curl -sk https://127.0.0.1:2087/healthz | grep -q ok || curl -sS http://127.0.0.1:2087/healthz | grep -q ok
