...
func TestDevInstallWritesHostStack(t *testing.T) {
	...
	if !contains(string(pdns), "local-address=127.0.0.1") {
		t.Fatalf("pdns.conf missing loopback listen: %s", pdns)
	}
	if contains(string(pdns), "local-address=127.0.0.1,203.0.113.10") || contains(string(pdns), "local-address=0.0.0.0") {
		t.Fatalf("pdns.conf must not bind unpublished NAT or wildcard: %s", pdns)
	}
