...
export function SecurityPage () {
	...
	async function reboot () {
		...
		try { await api('/api/v1/server/reboot', { method: 'POST', body: JSON.stringify({ confirm: 'REBOOT' }) }); setMessage('Host reboot was recorded and sent to the privileged agent.'); setRebootOpen(false); setConfirmation('') } catch (requestError) { setMessage(messageFrom(requestError)) }
	}
	return <>
		...
		<Dialog open={firewallOpen} title="Apply firewall configuration" onClose={() => { setFirewallOpen(false); setFirewallConfirmed(false) }}><p>This applies <code>{firewall?.table}</code> with a default inbound drop policy while retaining loopback, established flows, hosting services, and bound management ports.</p><label className="checkbox-label"><input type="checkbox" checked={firewallConfirmed} onChange={(event) => setFirewallConfirmed(event.target.checked)} /> I reviewed the active management path</label><footer className="dialog-form-actions"><button type="button" className="secondary" onClick={() => setFirewallOpen(false)}>Cancel</button><button type="button" disabled={!firewallConfirmed} onClick={applyFirewall}>Apply policy</button></footer></Dialog>
		<Dialog open={rebootOpen} title="Reboot host" onClose={() => { setRebootOpen(false); setConfirmation('') }}><p>Enter <strong>REBOOT</strong> to confirm this host-impacting action.</p><label>Confirmation<input autoFocus value={confirmation} onChange={(event) => setConfirmation(event.target.value)} /></label><footer className="dialog-form-actions"><button type="button" className="secondary" onClick={() => setRebootOpen(false)}>Cancel</button><button type="button" className="danger" disabled={confirmation !== 'REBOOT'} onClick={reboot}>Reboot host</button></footer></Dialog>
	</>
}
