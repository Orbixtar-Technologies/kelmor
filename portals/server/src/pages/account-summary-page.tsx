...
export function AccountSummaryPage () {
	...
	return (
		<>
			<PageHeader title={account.username} description={`${account.primary_domain} · ${account.status}`} actions={<>{canSuspend ? <button type="button" className="secondary" onClick={() => {
			...
			<div className="summary-grid">
				...
				<section className="panel"><h2>Identity</h2><dl className="detail-list">
				...
				<section className="panel danger-panel"><h2>Terminate account</h2><p>Permanently removes hosted services and the Linux identity.</p><div className="button-row">{canTerminate ? <button type="button" className="danger" onClick={openTerminate}>Terminate account</button> : <p className="subtle">Termination is unavailable to your role.</p>}</div></section>
			</div>
